...
use graph::data::subgraph::schema::{SubgraphManifestEntity, SubgraphStateEntity, SUBGRAPHS_ID};
use graph::prelude::*;
...
impl Store {
    ...
    fn apply_abort_unless_operation(
        ...
        // Execute query
        let actual_entities = self
            .execute_query(conn, query)
            .map_err(|e| format_err!("AbortUnless: query execution error: {}", e))?;

        // Extract IDs from entities
        let actual_entity_ids: Vec<String> = actual_entities
            .into_iter()
        ...
        if actual_entity_ids != entity_ids {
            ...
            return Err(format_err!("AbortUnless triggered abort"));
        }
    ...
    fn apply_entity_operation(
        ...
        match operation {
            ...
            EntityOperation::AbortUnless { query, entity_ids } => {
                self.apply_abort_unless_operation(conn, query, entity_ids, event_source)
            }
        }
    }
...
impl StoreTrait for Store {
    ...
    fn find(&self, query: EntityQuery) -> Result<Vec<Entity>, QueryExecutionError> {
        self.execute_query(&*self.conn.lock().unwrap(), query)
    }
    ...
    fn transact_block_operations(
        ...
        let mut operations = EntityOperation::fold(&operations);
